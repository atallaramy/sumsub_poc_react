...
async function makeAuthenticatedRequest<T = any>(
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      console.error('Sumsub API Error:', error.response?.status, error.response?.data);
      throw new Error(`Sumsub API error: ${error.response?.status} ${JSON.stringify(error.response?.data)}`);
    }
...
app.post('/api/access-token', async (req: Request<{}, AccessTokenResponse, AccessTokenRequestBody>, res: Response<AccessTokenResponse>) => {
  try {
    ...
    // Create applicant first, then generate SDK access token
